...
// 		$content = $this->db_push->field('device_client')->find(array('message_id' => $message_id));
		...
		if (is_ecjia_error($result)) {
			$this->showmessage($result->get_error_message(), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('push/admin/init',array(appid => $appid))));
		} else {
