...
use admin_nav_here;
use ecjia;
use Ecjia\App\Push\Controllers\AdminBase;
use ecjia_admin;
use ecjia_screen;
use RC_App;
use RC_DB;
...
class AdminEventsController extends AdminBase
{
	...
	public function __construct()
    {
		...
		RC_Script::enqueue_script('push_events', RC_App::apps_url('statics/js/push_events.js', $this->__FILE__), array(), false, 1);
		RC_Script::localize_script('push_events', 'js_lang_events', config('app-push::jslang.push_events_page'));
		
		RC_Script::enqueue_script('push_template', RC_App::apps_url('statics/js/push_template.js', $this->__FILE__), array(), false, 1);
	...
	public function init ()
    {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('消息事件', 'push')));
		$this->assign('ur_here', __('消息事件列表', 'push'));
	...
	public function all_open()
    {
		...
		return $this->showmessage(__('全部开启成功', 'push'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('push/admin_events/init')));
	}
