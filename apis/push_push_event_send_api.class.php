 ...
 * @author royalwang
 */
class push_push_event_send_api extends Component_Event_Api {
	...
	public function call(&$options) {	    
	    ...
	    $user_type = array_get($options, 'user_type');
	    $event     = array_get($options, 'event');
	    $priority  = array_get($options, 'priority', 1);
	    $value     = array_get($options, 'value', array());
	    $field     = array_get($options, 'field', array());
	    
	    $eventHandler = with(new \Ecjia\App\Push\EventFactory\EventFactory())->event($event);
	    ...
	    $result = \Ecjia\App\Push\PushManager::make()
               ->setTemplateModel(new \Ecjia\App\Push\Models\PushTemplateModel())
	           ->setEvent($eventHandler)
	           ->setPushUser(new Ecjia\App\Client\User($user_type, $user_id))
	           ->send($value, $field);
